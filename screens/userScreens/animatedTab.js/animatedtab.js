...
// console.log(data);
const Tab= React.forwardRef(({item, onItemPress, ind}, ref)=>{

    function changeImageOnPress () {
        onItemPress(ind);
    }

    return (
        <TouchableOpacity onPress={changeImageOnPress}>
            <View ref={ref}>
    ...
    return (
        <View style={{position: 'absolute', width, top: 75}}>
            <View ref={containerRef} style={{
                ...
                {data.map((item, index)=>{
                    return <Tab key={item.key} item={item} ref={item.ref} onItemPress={onItemPress} ind={index}/>
                })}
            </View>
