...
function DirectionScreen({ navigation, route, destLocation, originLocation }) {
    ...
    const [currentLoc, setcurrentLoc] = useState(originLocation);
    const [locationSubscription, setLocationSubscription] = useState(null);
    const markerRef = useRef(null);
    const [markerHeading, setMarkerHeading] = useState(0);
    
    // setcurrentLoc(originLocation);
  ...
  return (
    ...
    //   camera={mapStaticData.cameraOption(latlong, locationAltitude)}
      >
        <Marker coordinate={currentLoc} anchor={{x: 0.5, y: 0.5}} ref={markerRef} rotation={markerHeading} >
          <Image source={require('../../assets/map/car-marker-big.png')} style={{width: 20}} resizeMode="contain"/>
        </Marker>
        <Marker coordinate={destination} anchor={{x: 0.5, y: 1}}>
            {/* <Image source={require('../../assets/map/dest.png')} style={{width: 30}} resizeMode="contain"/> */}
        </Marker>
        <MapViewDirections
            origin={currentLoc}
